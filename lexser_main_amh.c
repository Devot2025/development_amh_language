...
Amh_Lex_Token_List* gen_amh_token_list(uintptr_t src_ini_size) {
...
void delete_amh_token_list(Amh_Lex_Token_List** src_amh_token_list) {
	...
	for (uintptr_t i = 0; i < tmp_amh_token_list->stack_token_list_size; ++i) {
...
void amh_token_list_call(Amh_Lex_Token_List* src_amh_token_list) {
	const char* amh_token_type_str[] = {
		"ope", "punch", "bin literal", "spe literal", "s str", "d str", "standard", "keyword", "iden", "null"
	};
	for (src_amh_token_list->stack_token_list_index = 0; src_amh_token_list->stack_token_list_index < src_amh_token_list->stack_token_list_size; src_amh_token_list->stack_token_list_index++) {
		printf("[idx %10llu|type %10s|token %10s]\n", src_amh_token_list->stack_token_list_index,
...
Amh_Lex_Token_List * start_lex_amh_code(const char* src_amh_code) {
	...
	Amh_Lex_Mode now_lex_mode = E_Amh_Lex_Pripro_Space;
	...
	while (*src_amh_code) {
		...
		if (now_lex_mode == E_Amh_Lex_Normal) {
			...
			if (byte_match_to_list_8(tmp_code_byte, ";(){}")) {
				...
				now_lex_mode = E_Amh_Lex_Pripro;
			}
			else if (tmp_code_byte == '@') {
				/*token iden*/
				/*comment mode*/
				now_lex_mode = E_Amh_Lex_Comment;
			}
			else if (tmp_code_byte == ' ') {
				append_lex_token_to_token_list(amh_token, stack_token, E_Amh_Lex_Token_Type_NULL);
			}
		...
		else if (now_lex_mode == E_Amh_Lex_Pripro) {
		}
		...
		else if (now_lex_mode == E_Amh_Lex_Pripro_Space) {

			/*
			* file start ~  pripro def(#) end
			*/
			if (tmp_code_byte != '#') {
				now_lex_mode = E_Amh_Lex_Normal;
				continue;
			...
			/*change to amh_pripro.h*/
			if (simple_strcmp(stack_token, "#module")) {
	...
	Amh_Lex_Token * tmp_amh_lex_token = smart_realloc(Amh_Lex_Token, amh_token->stack_token_list, amh_token->stack_token_list_size);
	if (tmp_amh_lex_token)amh_token->stack_token_list = tmp_amh_lex_token;
	return amh_token;
}
